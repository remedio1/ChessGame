...
public class GamePanel extends JPanel implements Runnable{
	...
	// setting the total size of the window
    ...
    private void update(){
    	...
    	if(promotion) {
        	...
        	if(mouse.pressed) {
        		// if active p is null check if you can pick up a piece
        		if(activeP == null) {
        			
        			for(Piece piece : simPieces) {
        				// if the mouse is on ally piece, pick up as the activeP
    ...
    private boolean isCheckmate() {
    	...
    	if(kingCanMove(king)) {
    		...
    		// check the position of the checking piece and the king is in check
    		...
    		if(colDiff == 0) {
    			...
    			// the checking is attacking diagonally
    			if(checkingP.row < king.row) {
    				// the checking piece is above the king
    				if(checkingP.col < king.col) {
    					// the checking piece is to to the upper left
    			...
    			if(checkingP.row > king.row) {
    				// the checking piece is below the king
    				if(checkingP.col < king.col) {
    					// the checking piece is to to the lower left
    					for(int col = checkingP.col, row = checkingP.row; col < king.col; col++, row--) {
    						for(Piece piece : simPieces) {
    							if(piece != king && piece.color != currentColor && piece.canMove(col, row)) {
    								return false;
